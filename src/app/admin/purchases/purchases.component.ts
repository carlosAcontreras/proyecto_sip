  ...
  operation_purchases() {
    $(function() {
      $(document).on("keyup", ".item_actividad .request_amount", function(
        ...
        let vlr_discount = (
          parseFloat(unit_value) - parseFloat(total_discount)
        ).toFixed(2);

        //iva de cada producto
        let total_iva = (
          parseFloat(vlr_discount) *
          parseFloat(iva) /
          100
        ).toFixed(2);

        // valor de cada producto con iva
        let total_vlrdiscount = (
          parseFloat(vlr_discount) + parseFloat(total_iva)
        ).toFixed(2);
        // cantidad por el valor unitario incluido el descuento
        let vlr_iva = (parseFloat(quantity) * parseFloat(vlr_discount)).toFixed(
          2
        );
    ...
    function total() {
      ...
      for (data of rawData) {
        ...
        let request_amount = isNaN(data.request_amount)
          ? 0
          : data.request_amount;
        ...
        // valor de cada material con el descuento
        let vlr_discount = (
          Number(unit_value) - Number(total_discount)
        ).toFixed(2);
        ...
        let vlr_subtotal = (
          Number(request_amount) * Number(vlr_discount)
        ).toFixed(2);
